...
template <typename T> class Fifo
{
	...
	public:
		 ...
		 *
		 * This exception is returned when attempting to retrieve an object from
		 * an empty queue with a timeout.
		 */
	...
	public:
		 ...
		 * If the operation fails, the method throws an EmptyException
		 * exception.
		 *
		 * @return the element popped from the queue
		 */
...
{
		Mutex::Lock lck(mtx);
		while (elements.size() == 0) { lck.wait(); }
...
{
		Mutex::Lock lck(mtx);
		if (elements.size() == 0) { lck.wait(timeout_ms); }
		if (elements.size() > 0)
		{
				T element = elements.front();
				elements.pop();
