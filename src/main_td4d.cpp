...
int main(int argc, char **argv)
{
		...
		constexpr unsigned int nTokens(25);
		Fifo<int>			   fifo;
		Mutex				   mutex;

		// Create the threads
		std::vector<FifoConsumerThread> consThreads(
			nCons, FifoConsumerThread(&fifo, &mutex));

		std::vector<FifoProducerThread> prodThreads(
			nProd, FifoProducerThread(&fifo, nTokens));
